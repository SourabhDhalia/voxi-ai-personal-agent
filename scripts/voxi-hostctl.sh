#!/usr/bin/env bash
# Voxi installed-bundle host control script.
#
...
# lifecycle of an installed Voxi host bundle rooted at
# ${VOXI_INSTALL_ROOT:-${HOME}/.voxi}.
#
# Supported actions:
#   --help                 Show this help
...
# user to ./deploy.sh in a repository checkout.

set -euo pipefail

ENTRYPOINT_NAME="$(basename "$0")"

HOST_BASE_DIR="${VOXI_INSTALL_ROOT:-${HOME}/.voxi}"
...
TEST_TOOL_NAME="voxi-tests"

PID_FILE="${RUN_DIR}/voxi-host.pid"
TOOL_EXECUTOR_PID_FILE="${RUN_DIR}/voxi-tool-executor-host.pid"
...
usage() {
  ...
  printf '%b\n' "${BOLD}Voxi Installed Host Control${NC}"
  printf '\n'
  printf '%b\n' "${CYAN}Usage:${NC}"
  printf '  %s [action]\n' "${ENTRYPOINT_NAME}"
  printf '\n'
  ...
  printf '\n'
  printf '%b\n' "${CYAN}Environment:${NC}"
  printf '  VOXI_INSTALL_ROOT   Override the install root (default ~/.voxi)\n'
  printf '  VOXI_SOCKET_PATH    Override the daemon IPC socket path\n'
  printf '\n'
  printf '%b\n' "${CYAN}Notes:${NC}"
  printf '  This interface manages an already-installed Voxi bundle. To build,\n'
  printf '  test, or reinstall Voxi from source, use ./deploy.sh in a\n'
...
reject_source_only() {
  local flag="$1"
  {
    ...
    printf 'Use ./deploy.sh from a Voxi repository checkout for build, test,\n'
...
require_installed_tree() {
  [[ -d "${HOST_BASE_DIR}" ]] \
    || fail "Install root not found: ${HOST_BASE_DIR} (set VOXI_INSTALL_ROOT if the bundle lives elsewhere)"
  [[ -d "${INSTALL_DIR}" ]] \
...
start_daemon() {
  ...
  export VOXI_DATA_DIR="${HOST_BASE_DIR}"
