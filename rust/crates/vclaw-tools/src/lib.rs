...
use vclaw_api::SurfaceDescriptor;
use vclaw_plugins::{
...
pub fn plugin_manifest_entries(plugin_name: impl Into<String>) -> Vec<ToolManifestEntry> {
    let plugin_name = plugin_name.into();
    vclaw_plugins::plugin_tool_manifests()
        .into_iter()
        .map(|manifest| {
...
mod tests {
    ...
    fn test_context() -> GlobalToolContext {
        ...
        GlobalToolContext {
            ...
            task_registry: vclaw_runtime::TaskRegistrySnapshot {
                active_tasks: vec![vclaw_runtime::TaskPacket {
                    task_id: "task-1".to_string(),
                    summary: "refresh manifests".to_string(),
                    priority: vclaw_runtime::TaskPriority::High,
                    labels: vec!["tools".to_string()],
                    status: vclaw_runtime::TaskStatus::Queued,
                    assignment: Some(vclaw_runtime::TaskAssignment {
            ...
            cron_registry: vclaw_runtime::TeamCronRegistry {
                entries: vec![vclaw_runtime::TeamCronEntry {
                    entry_id: "cron-1".to_string(),
                    schedule: "0 * * * *".to_string(),
                    task_name: "metadata.sync".to_string(),
                    lane_id: "maintenance".to_string(),
                    enabled: true,
                    priority: vclaw_runtime::TaskPriority::Normal,
            ...
            lsp_clients: vec![vclaw_runtime::LspClientSpec {
    ...
    fn permission_wrapper_allows_fetch_tool_and_records_decision() {
        ...
        let permissions =
            PermissionEnforcer::with_prompter(RecordingPrompter::with_decisions(vec![
                vclaw_runtime::PermissionPromptDecision::AllowOnce,
            ]));
        let mut executor =
    ...
    fn global_registry_composes_builtin_runtime_plugin_and_mcp_tools() {
        ...
        let plugin_tools = vclaw_plugins::plugin_tool_manifests()
            .into_iter()
            .map(|manifest| {
        ...
        let responses = VecDeque::from(vec![
            JsonRpcResponse::success(
                ...
                serde_json::to_value(McpInitializeResult {
                    protocol_version: vclaw_runtime::mcp::MCP_PROTOCOL_VERSION.to_string(),
