...
fn daemon_socket_path_override() -> Option<std::path::PathBuf> {
...
fn default_daemon_socket_path() -> std::path::PathBuf {
    let base = std::env::var("VOXI_DATA_DIR")
        .map(std::path::PathBuf::from)
        .unwrap_or_else(|_| {
            let home = std::env::var("HOME").unwrap_or_else(|_| "/tmp".to_string());
            std::path::PathBuf::from(home).join(".voxi")
        });
    base.join("run").join(SOCKET_NAME)
}
...
/// Voxi agent — safe Rust API.
///
/// # Example (Rust)
/// ```rust,no_run
/// use voxi::api::Voxi;
///
/// let mut agent = Voxi::new();
...
impl Voxi {
    ...
    pub fn initialize(&mut self) -> Result<(), String> {
        ...
        log::info!("Voxi API initialized ({} tools)", self.tools.len());
        Ok(())
    }
    ...
    fn connect_daemon() -> Result<UnixStream, String> {
        if let Some(path) = daemon_socket_path_override() {
            return UnixStream::connect(&path)
                .map_err(|error| format!("Failed to connect to daemon at '{}': {}", path.display(), error));
        }
        ...
        if connect_result < 0 {
            ...
            return Err(format!(
                "Failed to connect to daemon. Is voxi running? {}",
                error
            ));
        }
    ...
    /// Voxi-owned embedded descriptor directory.
