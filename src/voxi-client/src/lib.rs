//! voxi-client — C FFI layer for Voxi Agent.
//!
//! Exports `extern "C"` functions matching the declarations in
//! `include/voxi.h`. Each function is `#[no_mangle]` so the
//! linker produces stable C-compatible symbols.
//!
//! Thread safety: the opaque handle wraps `Arc<Mutex<Voxi>>`,
...
const VOXI_ERROR_LLM_FAILED: i32 = -6;
const VOXI_ERROR_TOOL_FAILED: i32 = -7;
const _VOXI_ERROR_NOT_SUPPORTED: i32 = -8;
...
/// Internal representation of the opaque `voxi_h` handle.
struct HandleInner {
    agent: api::Voxi,
}
...
#[no_mangle]
pub unsafe extern "C" fn voxi_create(handle: *mut *mut libc::c_void) -> i32 {
    if handle.is_null() {
        set_last_error("handle pointer is null");
        return VOXI_ERROR_INVALID_PARAMETER;
    }
...
#[no_mangle]
pub extern "C" fn voxi_initialize(handle: *mut libc::c_void) -> i32 {
...
#[no_mangle]
pub extern "C" fn voxi_destroy(handle: *mut libc::c_void) {
    if handle.is_null() {
        return;
    }
    ...
    // drops HandleInner → drops Voxi (whose Drop impl calls shutdown()).
...
#[no_mangle]
pub unsafe extern "C" fn voxi_process_prompt(
...
/// Async callback type matching voxi_response_cb in voxi.h
type ResponseCallback = unsafe extern "C" fn(*const c_char, i32, *mut libc::c_void);

#[no_mangle]
pub unsafe extern "C" fn voxi_process_prompt_async(
    handle: *mut libc::c_void,
    session_id: *const c_char,
    prompt: *const c_char,
        ...
        match result {
            Ok(resp) => {
                if let Ok(cs) = CString::new(resp) {
                    cb(cs.as_ptr(), VOXI_ERROR_NONE, ud_ptr);
                } else {
                    cb(std::ptr::null(), VOXI_ERROR_LLM_FAILED, ud_ptr);
                }
            }
            Err(_) => {
                cb(std::ptr::null(), VOXI_ERROR_LLM_FAILED, ud_ptr);
...
#[no_mangle]
pub unsafe extern "C" fn voxi_clear_session(
    handle: *mut libc::c_void,
    session_id: *const c_char,
) -> i32 {
    let ptr = handle as HandlePtr;
...
#[no_mangle]
pub extern "C" fn voxi_get_status(handle: *mut libc::c_void) -> *mut c_char {
    let ptr = handle as HandlePtr;
    if ptr.is_null() {
        set_last_error("handle is null");
        return std::ptr::null_mut();
...
#[no_mangle]
pub extern "C" fn voxi_get_metrics(handle: *mut libc::c_void) -> *mut c_char {
    let ptr = handle as HandlePtr;
    if ptr.is_null() {
        set_last_error("handle is null");
        return std::ptr::null_mut();
...
#[no_mangle]
pub extern "C" fn voxi_get_tools(handle: *mut libc::c_void) -> *mut c_char {
    let ptr = handle as HandlePtr;
    if ptr.is_null() {
        set_last_error("handle is null");
        return std::ptr::null_mut();
...
#[no_mangle]
pub unsafe extern "C" fn voxi_execute_tool(
    handle: *mut libc::c_void,
...
#[no_mangle]
pub extern "C" fn voxi_reload_skills(handle: *mut libc::c_void) -> i32 {
    let ptr = handle as HandlePtr;
    if ptr.is_null() {
        set_last_error("handle is null");
        return VOXI_ERROR_INVALID_PARAMETER;
...
#[no_mangle]
pub extern "C" fn voxi_start_dashboard(handle: *mut libc::c_void, port: u16) -> i32 {
    let ptr = handle as HandlePtr;
    if ptr.is_null() {
        set_last_error("handle is null");
        return VOXI_ERROR_INVALID_PARAMETER;
...
#[no_mangle]
pub extern "C" fn voxi_stop_dashboard(handle: *mut libc::c_void) -> i32 {
    let ptr = handle as HandlePtr;
    if ptr.is_null() {
        set_last_error("handle is null");
        return VOXI_ERROR_INVALID_PARAMETER;
...
#[no_mangle]
pub unsafe extern "C" fn voxi_free_string(ptr: *mut c_char) {
    if !ptr.is_null() {
...
#[no_mangle]
pub extern "C" fn voxi_last_error() -> *const c_char {
