...
#ifndef VOXI_CHANNEL_H_
#define VOXI_CHANNEL_H_
 ...
 *         returned string.
 */
typedef char* (*voxi_channel_prompt_cb)(
 ...
 */
bool VOXI_CHANNEL_INITIALIZE(const char* config_json);
 ...
 */
const char* VOXI_CHANNEL_GET_NAME(void);
 ...
 * @return @c true on success, @c false otherwise.
 */
bool VOXI_CHANNEL_START(
    voxi_channel_prompt_cb cb, void* user_data);

/**
 * @brief Stops and cleans up the channel plugin.
 */
void VOXI_CHANNEL_STOP(void);
 ...
 *         @c false otherwise.
 */
bool VOXI_CHANNEL_SEND_MESSAGE(
    const char* text);
