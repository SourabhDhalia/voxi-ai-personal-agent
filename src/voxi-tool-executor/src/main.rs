//! voxi-tool-executor — Asynchronous tool execution daemon.
//!
...
fn default_tools_dir() -> std::path::PathBuf {
    ...
    if std::path::Path::new("/opt/usr/share/voxi").exists() {
        return std::path::PathBuf::from("/opt/usr/share/voxi/tools");
    }
...
fn socket_path_override() -> Option<std::path::PathBuf> {
    std::env::var("VOXI_TOOL_EXECUTOR_SOCKET_PATH")
        .or_else(|_| std::env::var("VOXI_EXECUTOR_SOCKET_PATH"))
        .ok()
...
async fn handle_socket_client(stream: UnixStream) {
    ...
    if !peer_validator::validate(&stream, &["voxi", "voxi-cli"]) {
...
async fn main() {
    ...
    log::info!(
        "voxi-tool-executor starting (pid={})",
        std::process::id()
    );

    let (listener, socket_label, socket_path) = match systemd_socket() {
        Some(listener) => (listener, "systemd fd 3".to_string(), None),
        None => create_default_socket().expect("Failed to create socket"),
    };

    log::info!("Listening on socket: {}", socket_label);
    ...
    log::info!("voxi-tool-executor stopped");
...
fn create_default_socket() -> std::io::Result<(UnixListener, String, Option<std::path::PathBuf>)> {
    if let Some(path) = socket_path_override() {
        let listener = create_filesystem_socket(&path)?;
        return Ok((listener, path.display().to_string(), Some(path)));
    }
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        let listener = create_filesystem_socket(&path)?;
        Ok((listener, path.display().to_string(), Some(path)))
    }

    #[cfg(not(target_os = "macos"))]
    {
        let listener = create_abstract_socket()?;
        Ok((listener, format!("@{}", SOCKET_NAME), None))
    }
}

fn create_filesystem_socket(path: &std::path::Path) -> std::io::Result<UnixListener> {
