...
backend failures: `{}`",
    ...
    fn build_startup_message(state: &TelegramChatState) -> TelegramOutgoingMessage {
        TelegramOutgoingMessage::plain(format!(
            "Voxi agent started and Telegram channel is online.\nCurrent interaction mode: `{}`.\nCurrent session: `{}`.\nSelected CLI backend: `{}`.",
            state.interaction_mode.as_str(),
            state.active_session_label(),
            state.cli_backend.as_str()
        )
    ...
    fn build_cli_prompt(
        ...
        let mode_prefix = match execution_mode {
            TelegramExecutionMode::Plan => {
                "You are operating in Voxi Telegram coding mode. Start with a short plan, then perform the work carefully. Keep the final response concise and actionable."
            }
            TelegramExecutionMode::Fast => {
                "You are operating in Voxi Telegram coding mode. Optimize for speed, keep the response concise, and take the fastest reasonable path."
            }
        };
            ...
            tokio::spawn(async move {
                ...
                loop {
                    ...
                    if now.saturating_sub(last) >= IDLE_TRIM_SECS && last != trimmed_at {
                        ...
                        trimmed_at = last;
                        log::info!(
                            "TelegramClient: idle {}s; allocator idle trim checked",
...
mod tests {
    ...
    fn parse_command_handles_bot_mentions() {
        let parsed = TelegramClient::parse_command("/status@voxi_bot").unwrap();
    ...
    fn startup_message_mentions_current_mode() {
        ...
        assert!(message.text.contains("Voxi agent started and Telegram channel is online."));
        assert!(message.text.contains("Current session: `chat-0001`"));
        assert!(message.reply_markup.is_none());
    }
