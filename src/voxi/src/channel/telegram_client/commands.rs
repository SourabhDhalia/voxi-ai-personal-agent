    ...
    fn build_cli_prompt(
        ...
        let mode_prefix = match execution_mode {
            TelegramExecutionMode::Plan => {
                "You are operating as an AI agent via Voxi. Start with a short plan, then perform the work carefully. Keep the final response concise and actionable."
            }
            TelegramExecutionMode::Fast => {
                "You are operating as an AI agent via Voxi. Optimize for speed, keep the response concise, and take the fastest reasonable path."
            }
        };
...
User request:\n{}",
    ...
    fn build_tool_cli_prompt(
        ...
        let mode_prefix = match state.execution_mode {
            TelegramExecutionMode::Plan => {
                "You are an AI agent handling requests through Voxi. Start with a short plan, then perform the work carefully. Keep the final response concise and actionable."
            }
            TelegramExecutionMode::Fast => {
                "You are an AI agent handling requests through Voxi. Optimize for speed, keep the response concise, and take the fastest reasonable path."
            }
        };
    ...
    fn build_unified_agent_prompt(
        ...
        format!(
            "You are handling a Telegram request through Voxi.\n\
\n\
Runtime preferences:\n\
- Backend: {}\n\
- Model: {}\n\
