...
//!
//! On supported hosts, uses native Audio API (via FFI/libraries) for microphone capture
//! and speaker output. Speech-to-text (STT) is provided by the STT
//! framework, and text-to-speech (TTS) by the TTS framework.
//!
...
impl VoiceChannel {
    pub fn new(config: &ChannelConfig) -> Self {
        let stt = match config
            ...
            .unwrap_or("none")
        {
            "native" | "system" => SttEngine::SystemNative,
        ...
        let tts = match config
            ...
            .unwrap_or("none")
        {
            "native" | "system" => TtsEngine::SystemNative,
    ...
    fn speak(&self, text: &str) {
        match self.tts_engine {
            TtsEngine::SystemNative => {
                log::debug!(
                    "VoiceChannel: TTS(native) speak: {}",
                    utf8_safe_preview(text, 50)
                );
                // System TTS FFI/library call would go here
            }
    ...
    fn start(&mut self) -> bool {
        ...
        self.thread = Some(std::thread::spawn(move || {
            ...
            while running.load(Ordering::SeqCst) {
                ...
                match stt_engine {
                    SttEngine::SystemNative => {
                        // System STT FFI/library: capture mic → PCM → STT → text
...
mod tests {
    ...
    fn utf8_safe_preview_truncates_on_char_boundary() {
        let text = "Voxi™ ready";
        let preview = utf8_safe_preview(text, 4);
 
        assert_eq!(preview, "Voxi");
