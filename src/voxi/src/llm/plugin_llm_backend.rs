...
//! The plugin must export the C API defined in `voxi_llm_backend.h`:
//!   - `VOXI_LLM_BACKEND_INITIALIZE(config_json: *const c_char) -> bool`
//!   - `VOXI_LLM_BACKEND_GET_NAME() -> *const c_char`
...
// ─────────────────────────────────────────
//  C ABI callback type matching voxi_llm_backend.h
// ─────────────────────────────────────────
type ChunkCb = unsafe extern "C" fn(*const libc::c_char, *mut libc::c_void);

// ─────────────────────────────────────────
//  Plugin function signatures (match voxi_llm_backend.h exactly)
// ─────────────────────────────────────────
...
impl PluginLlmBackend {
    ...
    /// Build a `voxi_llm_messages_h` from Rust `LlmMessage` slice.
    unsafe fn build_messages_handle(messages: &[LlmMessage]) -> *mut libc::c_void {
        let mut messages_h: *mut libc::c_void = std::ptr::null_mut();
        voxi_core::llm_types::voxi_llm_messages_create(&mut messages_h);

        for msg in messages {
            let mut msg_h: *mut libc::c_void = std::ptr::null_mut();
            voxi_core::llm_types::voxi_llm_message_create(&mut msg_h);

            if let Ok(c_role) = CString::new(msg.role.as_str()) {
                voxi_core::llm_types::voxi_llm_message_set_role(
                    msg_h,
                    c_role.as_ptr(),
                );
            }
            if let Ok(c_text) = CString::new(msg.text.as_str()) {
                voxi_core::llm_types::voxi_llm_message_set_text(
                    msg_h,
                    c_text.as_ptr(),
                );
            }
            ...
            for tc in &msg.tool_calls {
                ...
                voxi_core::llm_types::voxi_llm_tool_call_create(&mut tc_h);
                if let Ok(c_id) = CString::new(tc.id.as_str()) {
                    voxi_core::llm_types::voxi_llm_tool_call_set_id(
                        tc_h,
                        c_id.as_ptr(),
                    );
                }
                if let Ok(c_name) = CString::new(tc.name.as_str()) {
                    voxi_core::llm_types::voxi_llm_tool_call_set_name(
                        tc_h,
                        c_name.as_ptr(),
                    );
                }
                ...
                if let Ok(c_args) = CString::new(args_str.as_str()) {
                    voxi_core::llm_types::voxi_llm_tool_call_set_args_json(
                        tc_h,
                        c_args.as_ptr(),
                    );
                }
                voxi_core::llm_types::voxi_llm_message_add_tool_call(msg_h, tc_h);
            }

            // Tool result message fields
            if !msg.tool_name.is_empty() {
                if let Ok(c_tn) = CString::new(msg.tool_name.as_str()) {
                    voxi_core::llm_types::voxi_llm_message_set_tool_name(
                        msg_h,
                        c_tn.as_ptr(),
                    );
                }
            ...
            if !msg.tool_call_id.is_empty() {
                if let Ok(c_tid) = CString::new(msg.tool_call_id.as_str()) {
                    voxi_core::llm_types::voxi_llm_message_set_tool_call_id(
                        msg_h,
                        c_tid.as_ptr(),
                    );
                }
            ...
            if !msg.tool_result.is_null() {
                ...
                if let Ok(c_tr) = CString::new(tr_str.as_str()) {
                    voxi_core::llm_types::voxi_llm_message_set_tool_result_json(
                        msg_h,
                        c_tr.as_ptr(),
                    );
                }
            ...
            voxi_core::llm_types::voxi_llm_messages_add(messages_h, msg_h);
        }

        messages_h
    }

    /// Build a `voxi_llm_tools_h` from Rust `LlmToolDecl` slice.
    unsafe fn build_tools_handle(tools: &[LlmToolDecl]) -> *mut libc::c_void {
        let mut tools_h: *mut libc::c_void = std::ptr::null_mut();
        voxi_core::llm_types::voxi_llm_tools_create(&mut tools_h);

        for tool in tools {
            let mut tool_h: *mut libc::c_void = std::ptr::null_mut();
            voxi_core::llm_types::voxi_llm_tool_create(&mut tool_h);

            if let Ok(c_name) = CString::new(tool.name.as_str()) {
                voxi_core::llm_types::voxi_llm_tool_set_name(tool_h, c_name.as_ptr());
            }
            if let Ok(c_desc) = CString::new(tool.description.as_str()) {
                voxi_core::llm_types::voxi_llm_tool_set_description(
                    tool_h,
                    c_desc.as_ptr(),
                );
            }
            ...
            if let Ok(c_params) = CString::new(params_str.as_str()) {
                voxi_core::llm_types::voxi_llm_tool_set_parameters_json(
                    tool_h,
                    c_params.as_ptr(),
                );
            }

            voxi_core::llm_types::voxi_llm_tools_add(tools_h, tool_h);
        }

        tools_h
    }

    /// Extract an `LlmResponse` from a `voxi_llm_response_h` handle.
    unsafe fn extract_response(response_h: *mut libc::c_void) -> LlmResponse {
        ...
        voxi_core::llm_types::voxi_llm_response_is_success(response_h, &mut success);
        resp.success = success;

        // text
        let mut text_ptr: *mut libc::c_char = std::ptr::null_mut();
        voxi_core::llm_types::voxi_llm_response_get_text(response_h, &mut text_ptr);
        if !text_ptr.is_null() {
        ...
        voxi_core::llm_types::voxi_llm_response_get_error_message(
            response_h,
            &mut err_ptr,
        );
        if !err_ptr.is_null() {
        ...
        voxi_core::llm_types::voxi_llm_response_get_prompt_tokens(
            response_h,
            &mut prompt_tokens,
        );
        voxi_core::llm_types::voxi_llm_response_get_completion_tokens(
            response_h,
            &mut completion_tokens,
        );
        voxi_core::llm_types::voxi_llm_response_get_total_tokens(
            response_h,
            &mut total_tokens,
        );
        voxi_core::llm_types::voxi_llm_response_get_http_status(
            response_h,
            &mut http_status,
        );
        ...
        unsafe extern "C" fn collect_tool_call(
            ...
            voxi_core::llm_types::voxi_llm_tool_call_get_id(tc_h, &mut id_ptr);
            voxi_core::llm_types::voxi_llm_tool_call_get_name(tc_h, &mut name_ptr);
            voxi_core::llm_types::voxi_llm_tool_call_get_args_json(
                tc_h,
                &mut args_ptr,
            );
        ...
        voxi_core::llm_types::voxi_llm_response_foreach_llm_tool_calls(
            response_h,
            collect_tool_call,
        ...
        voxi_core::llm_types::voxi_llm_response_destroy(response_h);
...
impl LlmBackend for PluginLlmBackend {
    fn initialize(&mut self, config: &Value) -> bool {
            ...
            self.resolve_symbol::<PluginInitFn>(handle, "VOXI_LLM_BACKEND_INITIALIZE")
        {
            ...
            if !result {
                log::error!(
                    "Plugin '{}' VOXI_LLM_BACKEND_INITIALIZE returned false",
                    self.name
                );
            }
            result
            ...
            log::warn!(
                "Plugin '{}' has no VOXI_LLM_BACKEND_INITIALIZE symbol, skipping init",
                self.name
            );
            true
        }
    ...
    async fn chat(
        ...
        unsafe {
            voxi_core::llm_types::voxi_llm_messages_destroy(messages_h);
            voxi_core::llm_types::voxi_llm_tools_destroy(tools_h);
        }
