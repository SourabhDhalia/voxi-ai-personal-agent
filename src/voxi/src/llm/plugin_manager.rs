...
impl PluginManager {
    ...
    pub fn scan_plugins(
        &mut self,
        pm: Option<&dyn voxi_core::framework::PackageManagerProvider>,
    ) {
        if let Some(pkgmgr) = pm {
            let pkgs = pkgmgr
                .get_packages_by_metadata_key("io.voxi.metadata.llm-backend");
            log::debug!(
            ...
            for pkg in pkgs {
                ...
                if let Some(so_name) = pkgmgr.get_package_metadata_value(
                    ...
                    if let Some(root_path) = pkgmgr.get_package_root_path(&pkg.pkg_id) {
                        ...
                        // Match Voxi string concatenation to prevent PathBuf absolute path wiping
    ...
    pub fn load_plugin_from_pkg(
        ...
        let so_value = match pkgmgr
            .get_package_metadata_value(pkgid, "io.voxi.metadata.llm-backend")
        {
            Some(v) => v,
