...
pub fn strip_frontmatter(content: &str) -> &str {
    ...
    if let Some(pos) = rest.find("\n---") {
        let after_dash = &rest[pos + 4..];
        return after_dash.trim();
    }
...
mod tests {
    ...
    fn prepare_skill_document_rebuilds_frontmatter() {
        let prepared = prepare_skill_document(
            "Battery Helper",
            "Handles battery checks for Voxi workflows.",
            "---\nname: old\ndescription: old\n---\n\n# Battery\nUse the device tool.",
        )
        .unwrap();
        ...
        assert!(prepared
            .document
            .contains("description: \"Handles battery checks for Voxi workflows.\""));
