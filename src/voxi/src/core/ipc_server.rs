...
impl IpcServer {
    ...
    fn default_socket_path() -> std::path::PathBuf {
        crate::core::runtime_paths::default_data_dir()
            .join("run")
            .join("voxi.sock")
    }

    fn bind_filesystem_socket(path: &std::path::Path) -> Result<i32, String> {
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent)
                .map_err(|err| format!("Failed to create IPC socket dir '{}': {}", parent.display(), err))?;
        }
        let _ = std::fs::remove_file(path);

        let fd = unsafe { libc::socket(libc::AF_UNIX, libc::SOCK_STREAM, 0) };
        if fd < 0 {
            return Err(format!("Failed to create IPC socket: {}", std::io::Error::last_os_error()));
        }
        ...
        if bytes.len() >= addr.sun_path.len() {
            ...
            return Err(format!("IPC socket path is too long: {}", path.display()));
        }
        ...
        if ret < 0 {
            ...
            return Err(format!("Failed to bind IPC socket '{}': {}", path.display(), error));
        }
    ...
    fn bind_default_socket() -> Result<(i32, String, Option<std::path::PathBuf>), String> {
        if let Some(path) = Self::socket_path_override() {
            let fd = Self::bind_filesystem_socket(&path)?;
            return Ok((fd, path.display().to_string(), Some(path)));
        }
        ...
        if ret < 0 {
            ...
            return Err(format!("Failed to bind IPC socket: {}", error));
        }

        Ok((fd, "@voxi.sock".to_string(), None))
    }

    #[cfg(target_os = "macos")]
    fn bind_default_socket() -> Result<(i32, String, Option<std::path::PathBuf>), String> {
        let path = Self::socket_path_override().unwrap_or_else(Self::default_socket_path);
        let fd = Self::bind_filesystem_socket(&path)?;
        Ok((fd, path.display().to_string(), Some(path)))
    }
    ...
    fn server_loop(
        ...
        let (sock, socket_label, socket_path) = match Self::bind_default_socket() {
            Ok(value) => value,
            Err(error) => {
                log::error!("{}", error);
                return;
        ...
        log::info!("IPC Server listening on {}", socket_label);
