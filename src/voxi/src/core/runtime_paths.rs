...
const DEFAULT_DASHBOARD_PORT: u16 = 9091;
...
impl RuntimeTopology {
    ...
    pub fn summary_json(&self) -> Value {
        let platform_paths = voxi_core::framework::paths::PlatformPaths::detect();
        json!({
            "runtime_root": platform_paths.data_dir.clone(),
            "packaged_root": platform_paths.docs_dir.clone(),
...
pub fn is_legacy_runtime() -> bool {
    false
}

pub fn default_data_dir() -> PathBuf {
    voxi_core::framework::paths::PlatformPaths::detect().data_dir
}

fn default_dashboard_port_for_runtime(_legacy_runtime: bool) -> u16 {
    DEFAULT_DASHBOARD_PORT
}

pub fn default_dashboard_port() -> u16 {
    default_dashboard_port_for_runtime(is_legacy_runtime())
}
...
pub fn default_tools_dir() -> PathBuf {
    voxi_core::framework::paths::PlatformPaths::detect().tools_dir
}
...
mod tests {
    ...
    #[test]
    fn default_dashboard_port_uses_default_in_legacy_runtime() {
        assert_eq!(
    ...
    #[test]
    fn default_dashboard_port_uses_default_on_host_runtime() {
        assert_eq!(
    ...
    fn default_dashboard_base_url_uses_localhost_with_default_port() {
        ...
        assert_eq!(url, format!("http://localhost:{}", DEFAULT_DASHBOARD_PORT));
    }

    #[test]
    fn runtime_topology_exposes_expected_contract_dirs() {
        let topology = RuntimeTopology::from_data_dir("/tmp/voxi-topology");

        assert_eq!(
            topology.config_dir,
            Path::new("/tmp/voxi-topology/config")
        );
        assert_eq!(
            topology.state_dir,
            Path::new("/tmp/voxi-topology/state")
        );
        assert_eq!(
            topology.registry_dir,
            Path::new("/tmp/voxi-topology/state/registry")
        );
        assert_eq!(
            topology.loop_state_dir,
            Path::new("/tmp/voxi-topology/state/loop")
        );
        assert_eq!(
            topology.telegram_sessions_dir,
            Path::new("/tmp/voxi-topology/telegram_sessions")
        );
    }
    ...
    fn runtime_topology_summary_includes_registry_dir() {
        ...
        assert_eq!(
            ...
            "/tmp/voxi-topology/state/registry"
        );
        assert_eq!(
            summary["loop_state_dir"],
            "/tmp/voxi-topology/state/loop"
        );
        assert_eq!(summary["tools_dir"], "/tmp/voxi-topology/tools");
    }
}
