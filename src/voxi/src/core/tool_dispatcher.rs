...
mod tests {
    ...
    use voxi_core::framework::paths::PlatformPaths;
    use serde_json::json;
    use std::fs;
    ...
    async fn execute_in_dir_respects_workdir() {
        ...
        // Canonicalize runtime.data_dir to handle symlinks on macOS
        let data_dir = runtime.data_dir.canonicalize().unwrap_or_else(|_| runtime.data_dir.clone());

        let result = dispatcher
            .execute_in_dir("pwd", &json!({}), None, Some(data_dir.as_path()))
            .await
            .unwrap();
        assert_eq!(
            result["cwd"],
