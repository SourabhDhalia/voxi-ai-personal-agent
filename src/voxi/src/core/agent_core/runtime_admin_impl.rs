impl AgentCore {
    ...
    async fn extract_and_save_memory(&self, history: &[LlmMessage], final_response: &str) {
        ...
        let scrub_paths = |val: &str| -> String {
            ...
            let voxi_dir_re = match regex::Regex::new(r"(?i)/root/\.voxi(?:/[a-zA-Z0-9._/-]+)*") {
                Ok(re) => re,
                Err(_) => return val.to_string(),
            };
            ...
            let res = voxi_dir_re.replace_all(&res, "[voxi_dir]");
            let res = user_home_re.replace_all(&res, "[home_dir]");
            res.into_owned()
        };

        let system_prompt = "You are an automated daemon component for Voxi responsible for extracting \
