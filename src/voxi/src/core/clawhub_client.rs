...
pub async fn clawhub_update(skill_hubs_dir: &Path) -> Result<Value, String> {
    ...
    for entry in &lock.skills {
        ...
        // operators using VOXI_CLAWHUB_URL or CLAWHUB_URL get consistent
...
pub fn skill_hubs_dir_from_paths(
    paths: &voxi_core::framework::paths::PlatformPaths,
) -> PathBuf {
...
mod tests {
    ...
    /// path must use the URL from `VOXI_CLAWHUB_URL` (or `CLAWHUB_URL`)
    ...
    fn clawhub_update_missing_source_url_falls_back_to_env_var() {
        ...
        unsafe { std::env::set_var("VOXI_CLAWHUB_URL", custom_url) };
        ...
        unsafe { std::env::remove_var("VOXI_CLAWHUB_URL") };
