//! Voxi Rust Daemon — full daemon entry point.
//!
...
async fn main() {
    ...
    let platform = voxi_core::framework::PlatformContext::detect();
    platform.paths.ensure_dirs();
    let boot_log_path = platform.paths.logs_dir.join("voxi.log");
    let mut boot_logger = common::boot_status_logger::BootStatusLogger::new(boot_log_path);

    // Fix OpenSSL TLS certificate path if standard file exists
    ...
    log::info!("[Boot] Voxi daemon ready.");
