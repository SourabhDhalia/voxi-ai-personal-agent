//! HTTP client using reqwest with CA certificate support.
//!
//! Provides GET/POST with retry, exponential backoff, and automatic
//! CA certificate discovery matching the C++ curl behavior.

use reqwest::{Certificate, Client, ClientBuilder};
use std::sync::OnceLock;

/// Standard CA certificate paths probed at startup.
...
/// Retrieve or initialize the shared reqwest HTTP client, configured for TLS.
...
fn build_agent(timeout_secs: u64) -> Client {
    ...
    // Probe for CA certificate file (matching C++ curl behavior)
