...
const EXECUTOR_BINARY: &str = "voxi-tool-executor";
...
impl ContainerEngine {
    ...
    fn default_executor_socket_path() -> std::path::PathBuf {
    ...
    async fn connect_ipc() -> Result<UnixStream, String> {
        if let Some(path) = Self::socket_path_override() {
            return UnixStream::connect(&path)
                .await
                .map_err(|e| format!("Failed to connect to executor socket '{}': {}", path.display(), e));
        }

        #[cfg(target_os = "macos")]
        {
            let path = Self::default_executor_socket_path();
            return UnixStream::connect(&path)
                .await
                .map_err(|e| format!("Failed to connect to executor socket '{}': {}", path.display(), e));
        }
        ...
        if ret < 0 {
            ...
            return Err("Failed to connect to voxi-tool-executor".into());
        }
    ...
    fn executor_binary_path() -> Result<std::path::PathBuf, String> {
        Self::executor_binary_candidates()
            ...
            .ok_or_else(|| "Failed to locate voxi-tool-executor binary".to_string())
    }
...
mod tests {
    ...
    fn executor_candidates_include_system_binary() {
        ...
        assert!(candidates
            .iter()
            .any(|path| path == std::path::Path::new("/usr/bin/voxi-tool-executor")));
    }
}
