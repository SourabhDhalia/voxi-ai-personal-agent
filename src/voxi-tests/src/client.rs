...
const DEFAULT_SOCKET_NAME: &str = "voxi.sock";
...
fn default_socket_path() -> std::path::PathBuf {
    ...
    base.join("run").join(DEFAULT_SOCKET_NAME)
}
...
impl IpcClient {
    ...
    fn connect(&self) -> Result<UnixStream, String> {
        let socket_path_env = std::env::var("VOXI_SOCKET_PATH").ok();
        if let Some(path) = self.options.socket_path.as_deref().or(socket_path_env.as_deref()).filter(|value| !value.trim().is_empty()) {
        ...
        let socket_name = self
            ...
            .socket_name
            .clone()
            .or_else(|| std::env::var("VOXI_IPC_SOCKET_NAME").ok())
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            let stream = UnixStream::connect(&path)
                .map_err(|err| Self::format_connect_error(&format!("socket '{}'", path.display()), &err))?;
            Self::configure_stream(&stream)?;
            Ok(stream)
        }
    ...
    fn format_connect_error(target: &str, err: &std::io::Error) -> String {
        let guidance = match err.kind() {
            ErrorKind::NotFound | ErrorKind::ConnectionRefused | ErrorKind::TimedOut => {
                " Is the Voxi daemon running? Start it and retry."
            }
            _ => ""
        };
