//! voxi-web-dashboard — standalone HTTP dashboard binary.
//!
//! Serves the Voxi web UI and REST API. Communicates with the main
//! voxi daemon via IPC (Unix abstract socket) for chat requests.
//!
//! Usage:
//!   voxi-web-dashboard [--port 9091] [--web-root PATH]
...
fn default_data_dir() -> PathBuf {
    ...
    if std::path::Path::new("/opt/usr/share/voxi").exists() {
        return PathBuf::from("/opt/usr/share/voxi");
    }
...
async fn api_agent_card() -> Json<Value> {
    Json(json!({
        "name": "Voxi Agent",
        "description": "Voxi AI Agent System for Linux/Ubuntu",
