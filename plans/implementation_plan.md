...
This plan outlines how to resolve the heavy 30-minute Voxi/GBS build time for the Voxi agent while preserving all codebase packages, FFI client libraries, security metadata plugins, and the full range of autonomous agent tools and capabilities.
...
> 3. SQLite (`rusqlite`) will remain set to `features = ["bundled"]`. This ensures SQLite compiles cleanly across all environments (Voxi TV and Ubuntu) without requiring additional system-level development packages, while only adding ~1 minute to the overall compilation.

> [!TIP]
> **Scalability & Capabilities Maintained**:
> - All workspace members (such as `src/libvoxi` for C FFI client access and the `voxi-metadata-*` packaging hook plugins) are preserved.
...
#### [MODIFY] [libvoxi-core Cargo.toml](file:///Users/sdhalia/Developer/githubRepo/voxi-rust/src/libvoxi-core/Cargo.toml)
- Replace `native-tls-vendored` with `native-tls` in the `reqwest` dependency.

#### [MODIFY] [voxi Cargo.toml](file:///Users/sdhalia/Developer/githubRepo/voxi-rust/src/voxi/Cargo.toml)
- Replace `native-tls-vendored` with `native-tls` in the `reqwest` dependency.
...
3. Validate client access via `libvoxi.so` or `voxi-cli`.
