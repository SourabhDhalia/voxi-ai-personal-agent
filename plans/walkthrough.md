...
- **Dynamic OpenSSL Linking**: Replaced `native-tls-vendored` with standard dynamic `native-tls` in `src/voxi/Cargo.toml` and `src/libvoxi-core/Cargo.toml`.
...
- Successfully pushed the new commit (`7ecc6d57`) to the remote repository `origin/main` at `https://github.com/SourabhDhalia/voxi-rust.git`.
